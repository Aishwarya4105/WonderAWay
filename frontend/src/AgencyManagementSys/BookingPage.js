...
/*const BookingPage = () => {
  ...
  useEffect(() => {
    ...
    // MINIMAL FIX 1: Check for the string "undefined" specifically
  ...
  const handleSubmit = async (e) => {
    ...
    const bookingPayload = {
      package_id: packageData?._id, // MINIMAL FIX 2: Added ?
      packageName: packageData?.title, // MINIMAL FIX 3: Added ?
      price: packageData?.price,
      userName: formData.userName,
      email: formData.email,
      travelDate: formData.travelDate,
  ...
  return (
    <div className="booking-page">
      ...
      <div className="package-summary">
        {/* MINIMAL FIX 4: Added ? to these three lines 
        <p><strong>Package:</strong> {packageData?.title}</p>
        <p><strong>Duration:</strong> {packageData?.days} Days</p>
        <p><strong>Price:</strong> ₹{packageData?.price}</p>
      </div>
